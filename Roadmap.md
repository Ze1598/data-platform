...
- **`run_audit_log`** (per-layer run/audit history — idempotency + re-run support): run_id (uuid pk), layer (landing/raw/clean/staging/model/serve), feed_type, data_feed_id/model_feed_id (exactly one set), parent_run_id (fk, self — lineage), watermark_value_start/end, output_path, status, started_at/ended_at, rows_read/inserted/updated/deleted, error_message, dagster_run_id, created_at — indexed for "latest run per feed" lookups
...
data-platform/
  ...
  metadata/                        # module: platform config DB (source_system/data_feed/model_feed/run_audit_log)
...
5. **Dagster wiring (stubbed extraction)** — `dagster dev` locally + `K8sRunLauncher` for op pods; `dagster-dbt` loads the Phase 4 project; stub landing/raw/clean assets feed the real dbt staging asset; wire `run_audit_log` writes.
...
10. **(Stretch) Azure config swap proof** — two-sided swap against a real ADLS Gen2 account: update Trino's `iceberg.properties` (drop the `hive.s3.*`/MinIO properties for `fs.azure.enabled=true` + `azure.oauth.*`), and re-register the Polaris catalog's storage config from `S3` to `AZURE` (`default-base-location=abfss://...`, tenant/app credentials). Validates the portability claim end-to-end.
...
- Phases 5–6: trigger a Dagster asset materialization from the UI/CLI, confirm pods launch in kind (`kubectl get pods`), confirm `run_audit_log` rows are written with correct status/row counts.
- Phase 9: full pipeline run against one real source end-to-end, then a deliberate mid-pipeline failure to confirm re-run picks up cleanly from `run_audit_log` state.
