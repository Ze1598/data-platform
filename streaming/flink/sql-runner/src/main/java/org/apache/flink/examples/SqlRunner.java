...
// Vendored verbatim from apache/flink-kubernetes-operator's own
// examples/flink-sql-runner-example (Apache-2.0) -- the operator project's
// reference mechanism for running a Flink SQL script as a FlinkDeployment
// application-mode job, with no per-pipeline Java code. This file is
// generic (reads any .sql file, executes its statements in order) and is
// not modified for this platform's specific pipeline -- see
// ../../sql-scripts/ for the actual Kafka->Iceberg logic, which is plain
// SQL, not Java.
