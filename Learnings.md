...
**Confirmed live, not just by reading `workspace.yaml`'s `grpc_server` config**: added a one-line, uniquely-tagged `context.log.info(...)` probe to a sensor function body, rebuilt the image, restarted *only* `dagster-code-server` (`dagster-daemon`'s pod was never touched — same pod, same age, throughout), then triggered a real `master_pipeline` run via `verify-sensor` while streaming `dagster-code-server`'s own logs to a file. The probe line appeared in `dagster-code-server`'s log output, correctly attributed to the sensor's own evaluation, and the daemon correctly received and acted on the result via gRPC — despite never running the updated code itself.
...
### Dagster control plane scaling on Kubernetes — scaling to zero was tried, then abandoned in favor of Dagster's own documented pattern

**Context**: onboarding a second streaming source surfaced that `orchestration`'s three Deployments (webserver, daemon, code-server) ran 24/7 with no resource limits, consuming real memory whether or not anyone was using Dagster. To address the idle-cost question, webserver and code-server were put behind KEDA's Cron scaler, scaling both to `minReplicaCount: 0` outside two configured schedule windows, with a cooperative "wake" step (a Kubernetes annotation) run before every trigger path (Streamlit, host-CLI verification recipes) to bring them back up on demand.

**What went wrong**: deciding "is it now safe to scale the control plane back to zero" required a heuristic, since a run only becomes visible in Dagster's own run storage once it's actually submitted — a wake-timestamp grace period (60s) plus "is any other `master_pipeline` run currently non-terminal," inferred from *other* runs' terminal-status events rather than an authoritative signal. That heuristic has a structural blind spot: a sleep-sensor tick evaluating one run's completion can fire in the exact gap before a different, about-to-be-submitted run exists anywhere in Dagster's storage — unpausing (and KEDA then scaling to zero) while that other run's own GraphQL submission/poll is genuinely in flight. Confirmed live, repeatedly, as `ConnectionResetError`/`TransportConnectionFailed` — including inside this project's own `verify-pipeline` recipe, whose three sequential `master_pipeline` invocations after a single wake reliably outlasted any fixed grace-period guess.

**Resolution**: checked against Dagster's own documented, production Kubernetes deployment pattern (its Helm chart, published multi-code-location scaling case studies) rather than continuing to harden the heuristic — none of them scale webserver/daemon/code-server to zero; these are treated as small, always-on singleton Deployments. Real elasticity in a production Dagster-on-Kubernetes deployment happens one layer down (ephemeral per-run Job pods — already how this project's `EXTRACTION_JOBS`/`MODELING_JOBS`/`SERVING_JOBS` work, unaffected by this) and one layer further down still (node/cluster-autoscaler shrinking the underlying node pool, not application pods). Reverted to that pattern: `dagster-webserver`/`dagster-code-server`/`dagster-daemon` are all plain `replicas: 1`, always-on, no KEDA involved, and the cooperative wake/sleep mechanism (annotations, the run-status sensor, the Streamlit-side wake call) was removed entirely rather than kept as a fallback.

**Caveat**: if idle control-plane cost genuinely needs to shrink on a real cloud deployment later, the production-grade way to get scale-to-zero for a singleton HTTP/API service is a request-queueing proxy in front of it (KEDA's HTTP Add-on, or Knative's Activator) that buffers/holds a request during a cold start rather than passing it through to a pod that isn't there yet. That's a materially larger, separate piece of infrastructure — not a smaller version of the annotation-based approach tried here, and not something to reach for reflexively without that queueing layer.

### `kubectl delete -f --ignore-not-found` doesn't cover a CRD that was never installed

**Symptom**: `streaming/flink::kill` (run under `set -euo pipefail`) aborted with `error: Recipe kill failed with exit code 1` on the very first `just smoketest` run against a genuinely fresh cluster (Docker itself wasn't even running yet) — `kubectl delete -f generated/ --ignore-not-found` (against a generated `FlinkDeployment` manifest) printed `no matches for kind "FlinkDeployment"` ... `ensure CRDs are installed first`, and the script died right there — before ever reaching the `helm uninstall ... || true` line immediately below it.

**Cause**: `--ignore-not-found` only suppresses "no object with this name exists" — it does *not* suppress "this API kind isn't registered at all," a different, harder kubectl error that happens when the CRD providing that kind (installed by `helm install flink-kubernetes-operator`, a separate step from `kill`) was never installed in this cluster. The recipe already anticipated the *same* "operator/CRD never installed" scenario for its `helm uninstall` line (`|| true`) but missed the identical case one line above it for the `kubectl delete -f <manifest-referencing-the-CRD-kind>` line.

**Resolution**: added `|| true` to the `kubectl delete -f ... --ignore-not-found` line, matching the guard already present on the very next line. Verified live against the exact failure condition (a freshly created `kind` cluster, before the Flink Operator was ever installed): the top-level `just kill` now runs the full module chain to completion (`EXIT_CODE:0`) instead of aborting on the first CRD-based module it reaches.

**Caveat/generalizable lesson**: this only affects a `kill` recipe that `kubectl delete -f`s a manifest referencing a CRD-provided kind (Flink Operator's `FlinkDeployment`) — every other module's `kill` recipe in this repo only references built-in kinds (Deployment/Service/Secret/StatefulSet/PVC/ConfigMap/Job), which are always registered regardless of what's installed, so `--ignore-not-found` alone is sufficient there. Any *new* module that introduces its own CRD-based custom resource needs the same `|| true` on its `kill` recipe's delete-by-manifest line, or it inherits this exact gap.
...
**Symptom**: `frontend/pages/6_Trigger_Pipeline.py`'s (numbered `5_` at the time) `batch_group` picker populated its dropdown from `data_feed.batch_group` — a `uuid` column — instead of the separate `data_feed.batch_group_friendly_name` text column. Submitting a `batch_group` trigger returned `LaunchRunSuccess` with a real `runId` every time — no error, no exception, the exact response a correct submission would give — but every launched run then failed *inside* Dagster with `Failure: No active feeds resolved for orchestration_kind='batch_group' orchestration_value='<uuid>'`, because `PostgresMetadataResource.get_batch_group_feeds_by_tier()` matches `WHERE batch_group_friendly_name = %s`, and a uuid string matches no row.
