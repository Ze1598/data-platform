...
**Resolution**: added `PostgresMetadataResource.record_run_started()`, called explicitly as the very first action of a new `pipeline_init_<feed>` asset that runs before any extraction logic — every feed's run gets registered up front, deterministically, not as a side effect of whichever stage happens to log first. This is also what makes the master pipeline a real Dagster construct rather than just descriptive prose: `pipeline_init_<feed>` is a genuine upstream dependency every other stage-owning asset in that feed's chain depends on.
