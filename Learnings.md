...
Continue work on the data platform — see Roadmap.md/Progress.md/Backlog.md/Learnings.md/CLAUDE.md for context, and .claude/plans/ for pending work from previous sessions if one exists. If you find anything unexpected while working, stop and raise it before proceeding.
...
**Cause**: `raw_customers` returns `Output(None, metadata={...})` (Roadmap.md "Master pipeline orchestration" — raw's real durable output is the parquet file on disk, not an in-memory value; clean reads that file back itself rather than accepting a passed value, matching the "read storage from the previous layer" principle applied uniformly). Declaring the downstream asset's `raw_customers` parameter with a `None` type annotation was meant purely to establish execution ordering, not to receive a real value — but Dagster's IO manager treats an upstream `Output(None)` as "nothing to load," and doesn't pass anything at all for that parameter, rather than passing `None` itself. Python's own call then fails on a required positional argument with nothing supplied.
...
Real production Dagster deployments (and now this project's own, see Roadmap.md "Master pipeline orchestration") split into three independent pieces rather than `dagster dev`'s single collapsed process:
...
**Caveat, stated plainly, not glossed over**: this project's own verification tooling (`verify-pipeline`/`verify-schedule`/`verify-sensor`) now has a cooperative-wake mechanism (`_wake-orchestration`/`_sleep-orchestration`, see next entry), but Streamlit itself still doesn't — see Backlog.md, "Cooperative wake-up mechanism for Dagster from Streamlit." A manual trigger attempted from the frontend while `orchestration` is scaled to zero will still fail/hang until the next scheduled window until that feature exists.
...
**Caveat**: this is a stable, documented KEDA feature (not a workaround) — but it only solves the *test tooling's* cooperative wake. The real user-facing case (a Streamlit-triggered Dagster run) still needs the same pause-then-scale pattern built into that future feature, not just this recipe; see Backlog.md.
