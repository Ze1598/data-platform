...
from dagster_data_platform.assets.extraction_assets import FEED_POOL
...
class DataPlatformDbtTranslator(DagsterDbtTranslator):
    """Maps the dbt source `clean.customers` onto the same asset key the
    stub `clean_customers` asset (extraction_assets.py) produces, so the
    landing -> raw -> clean chain and dbt's staging model share one asset
    graph instead of two coincidentally-ordered ones."""

    def get_asset_key(self, dbt_resource_props: Mapping[str, Any]) -> AssetKey:
        if (
            dbt_resource_props["resource_type"] == "source"
            and dbt_resource_props["source_name"] == "clean"
            and dbt_resource_props["name"] == "customers"
        ):
            return AssetKey("clean_customers")
...
def data_platform_dbt_assets(context: AssetExecutionContext, dbt: DbtCliResource, postgres_metadata: PostgresMetadataResource):
    data_feed = postgres_metadata.get_data_feed("customers")
    with postgres_metadata.log_ingestion_step(
        ...
        if not invocation.is_successful():
            raise invocation.get_error() or RuntimeError("dbt build failed")
