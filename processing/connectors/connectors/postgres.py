...
discover_schema() is scoped to sample-inference (the shared
TabularConnector default) rather than true information_schema/pg_catalog
lookup -- true catalog-based discovery for a single-table source is a
...
shape/type inference required), so it doesn't hit the "query may span more
than one table" problem the same way column discovery would; a caller
simply doesn't supply table_name for a genuinely multi-table custom query
(see the ODS design, Roadmap.md).
